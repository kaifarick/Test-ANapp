...
namespace Mopsicus.InfiniteScrollMod {

	/// <summary>
	/// Infinite scroller for long lists
	/// </summary>
	public class InfiniteScrollMod : MonoBehaviour, IDropHandler {
		...
		void UpdateVertical () {
			...
			int position = (Mathf.Abs(_topPosition) > itemPosition) ? _previousPosition + 1 : _previousPosition - 1;
			int border = (int) (_positions[0] + _heights[0]);
			int step = Mathf.Abs((int) ((_topPosition + _topPosition / 1.25f) / border));
		...
		void CreateViewsVertical () {
			...
			int fillCount = Mathf.RoundToInt(_container.height / height) + _poolSize;
